...
class Config:
    ...
    def get_caffe_train_baseweights(self):
        path = os.path.expandvars(os.path.expanduser(self.caffe_train_baseweights))
        return path
    ...
    def load_value(self, reader, field_name, default_value=None, save_missing=False):
        if reader.has_option(Config.SectionName, field_name):
            if isinstance(default_value, str):
                ...
                assert(isinstance(default_value[0], str)) # Only lists of strings supported (comma-separated)
